...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        that.show = function (prevAnim) {
            ...
            var html = '<div class="dw-trans ' + s.theme + ' dw-' + s.display + '">' + (s.display == 'inline' ? '<div class="dw dwbg dwi"><div class="dwwr">' : '<div class="dw-persp">' + '<div class="dwo"></div><div class="dw dwbg ' + mAnim + '"><div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div><div class="dwwr"><h2>日期选择</h2>' + (s.headerText ? '' : ''));

            for (i = 0; i < s.wheels.length; i++) {
                html += '<div class="dwc' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '"><div class="dwwc dwrc"><table cellpadding="0" cellspacing="0" style="width:374px;"><tr>';
                // Create wheels
                for (label in s.wheels[i]) {
                    warr[l] = s.wheels[i][label];
                    html += '<td style="padding-right:34px;display:inline-block;"><div class="dwwl dwrc dwwl' + l + '">' + (s.mode != 'scroller' ? '<div class="dwwb dwwbp" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>&ndash;</span></div>' : '') + '<div class="dwl">' + label + '</div><div class="dww" style="height:' + (s.rows * hi) + 'px;min-width:' + s.width + 'px;"><div class="dw-ul">';
                    // Create wheel values
            ...
            if (s.display != 'inline') {
                ...
                that.tap($('.dwb-s span', dw), function () {
                    if (that.hide(false, 'set') !== false) {
                        ...
                        Y_DateTime(30,endTime);
                        var appDate=$('#appDate').val();
                        var appDate1=$('#appDate1').val();
                        var startTime = (new Date(appDate)).getTime();//传过来的开始时间转换为毫秒
                        var endTime = (new Date(appDate1)).getTime()
                        if(startTime>endTime){
                            /*alert('日期选择错误')*/
                            showalert(0,'日期选择错误',2);
                            if(appDate==that.val){
                                $('#appDate').val('')
                            }else{
                                $('#appDate1').val('')
                            }
                        }else if(parseInt(Math.abs(startTime - endTime ) / 1000 / 60 / 60 /24)>30){
                           /* alert('最大可选时间段为30天');*/
                            showalert(0,'最大可选时间段为30天',2);
                            if(appDate==that.val){
                                $('#appDate').val('')
                            }else{
                                $('#appDate1').val('')
