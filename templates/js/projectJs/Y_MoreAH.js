...
function visitor(lastYear,pageNum){
    YData={
        'pathL':"/health/bodyTemperature/listByPageVistor",
        'familyId':familyId,
        'accessToken':token,
    }
...
function Notvisitor(lastYear,pageNum){
    YData={
        'pathL':"/health/bodyTemperature/listByPage",
        'accessToken':token,
        'userId':uid,
    ...
    Ajax({
        ...
        success:function(res){
            if(res.state==0){
                ...
                var html= new EJS({url:"../compileEjs/Y_MoreAH.ejs"}).render({data:data,nowYear:year,lastYear:lastYear});
