$(function(){
   ...
   if(ifNew!=''&& ifNew=='1'){
        var data=null;
        var topHtml= new EJS({url:"../compileEjs/J_docAdviceEdit_2.ejs"}).render({data:data,memberId:memberId,applyFormId:applyFormId});
    ...
    function loadAction(){
        ...
        Ajax({
            ...
            success:function(res){
                   /* var res={"data":[],"msg":"成功","state":0};*/
                    var data=res.data;
                    var state=res.state;
                    var topHtml= new EJS({url:"../compileEjs/J_docAdviceEdit_2.ejs"}).render({data:data,memberId:memberId,applyFormId:applyFormId,state:state,ifPatient:ifPatient,myPatient:myPatient});
                    $('.TNB_form').html(topHtml);
                    $('.JindexBox').css('height',$(window).height()-$('.Ynav').height());
    ...
    function initData(data){
      ...
      //  $('.selectH4').find('input').val('');
        $('.selectH4').find('input').attr('disabled','disabled');
        $(isId).find('input').removeAttr('disabled');
        $('.selectH4').hide();
        $(isId).show();
        //用药添加class，方便序列化时候
        $('.isTap3 input,.isTap4 input').addClass('flau_u_ro');
        //初始化具体药物选择
    ...
    function methodThing(){
        ...
        $('.TNB_medTypes .TNB_mTypeOk').on('tap',function(){
            ...
            $('.medicineType').val($('.TNB_mTypeSelInp').val());
            $('.selectH4').find('input').val('');
            $('.selectH4').find('input').attr('disabled','disabled');
            $(isId).find('input').removeAttr('disabled');
            $('.selectH4').hide();
    ...
    function temSave(Ibtn){
        if($('.askReason').val().length<=300){
            if($('.TNB_advTTextarea').val().length<=500){
                ...
                $('.selectH4').each(function(i){
                    if($('.selectH4').eq(i).css('display')=='block'){
                        var This=$('.selectH4').eq(i);
                        This.find('.select_itemMadic').each(function(j){
                            var iName=This.find('.select_itemMadic').eq(j).find('.ajaxInp').attr('name');
                            if(This.find('.select_itemMadic').eq(j).find('h5 i').hasClass('checked')){ //如果选中了
                                var imorning=This.find('.select_itemMadic').eq(j).find('.morning_inp').val()==''?null:This.find('.select_itemMadic').eq(j).find('.morning_inp').val();
                                var inoon=This.find('.select_itemMadic').eq(j).find('.noon_inp').val()==''?null:This.find('.select_itemMadic').eq(j).find('.noon_inp').val();;
                                var inight=This.find('.select_itemMadic').eq(j).find('.night_inp').val()==''?null:This.find('.select_itemMadic').eq(j).find('.night_inp').val();
                                medicine[''+iName+'']='{"morning":'+imorning+',"noon":'+inoon+',"night":'+inight+'}';
                            }else{
                                medicine[''+iName+'']='';
                            }
                        })
                ...
                if(Ibtn==1){
                    iUrl={
                        'pathL':"/doctor/diabetesTemplet/addDiabetesTemplet",
                        'accessToken':token,
                        'sendFlag':1,
                    };
                }else{
                ...
                var ajaxData = $.extend({},isData,medicine,iUrl);
