...
/*function visitor(lastYear,pageNum){
    YData={
        'pathL':"/health/urinalysis/listByPageVist  or",
        'familyId':familyId,
        'accessToken':token,
...
function Notvisitor(lastYear,pageNum){
    YData={
        'pathL':"/health/urinalysis/listByPage",
        'accessToken':token,
        'userId':uid,
        'pageNum':parseInt(pageNum)+1,
        'pageSize':20
    ...
    Ajax({
        ...
        success:function(res){
            if(res.state==0){
                ...
                var html= new EJS({url:"../compileEjs/Y_MoreUL.ejs"}).render({data:data,nowYear:year,lastYear:lastYear});
